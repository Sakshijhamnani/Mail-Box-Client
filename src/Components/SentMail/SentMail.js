...
const fetchData = async () => {
    ...
    const response = await fetch(`https://mail-client-box-5531a-default-rtdb.firebaseio.com/${newEmail}sent.json`);
      ...
      const newEmail = email.replace(/[^\w\s]/gi, '');
       ...
       try {
         const response = await fetch(
           `https://mail-client-box-5531a-default-rtdb.firebaseio.com/${newEmail}sent/${id}.json`,
           {
             method: "DELETE",
             headers: {
        ...
        <ul className={classes.ul}>

            {
                items.map((item,index)=>(
                    ...
                    <p  style={{marginLeft: "90rem", zIndex: "1",cursor:"pointer"}} onClick={()=>deleteMailHandler(item.idd)}>🗑️</p>
                  </li>
                ))
            }
